...
# Fullscreen centered layout without global scrollbar
with ui.row().classes("w-screen h-screen overflow-hidden justify-center bg-white"):
    with ui.column().classes("w-full max-w-5xl h-full"):
        # Chat card taking full height
        with ui.card().classes(
            "w-full h-full flex flex-col justify-between shadow-sm rounded-xl bg-white"
        ):
            # Header
            ui.label("CRE-Suite").classes("text-2xl text-center text-gray-800 py-2")

            # Scrollable chat area only
            with ui.scroll_area().classes("flex-1 px-4 overflow-y-auto") as chat_area:
